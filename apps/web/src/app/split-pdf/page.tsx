...
import {
  FileText, Scissors, Loader2, Plus, X, Check,
  Layers, AlignLeft,
} from 'lucide-react';
...
export default function SplitPdfPage() {
  ...
  async function loadFile(pdfFile: File) {
    ...
    try {
      ...
      // Use CDN worker so we don't need to copy the worker file
      ...
      for (let i = 1; i <= count; i++) {
        ...
        setThumbs(prev => prev.map(t => t.pageNum === i ? { ...t, dataUrl } : t));
        setThumbsLoaded(i);
      }
    } catch {
      toast.error('Failed to read PDF — file may be corrupted or encrypted');
      setFile(null);
    } finally {
  ...
  const {
    ...
    onDrop, accept: ACCEPTED, maxSize: MAX_SIZE, maxFiles: 1, onDropRejected,
  });

  // ── Thumbnail helpers ─────────────────────────────────────────────────────

  function togglePage(pageNum: number) {
    setThumbs(prev => prev.map(t => t.pageNum === pageNum ? { ...t, selected: !t.selected } : t));
  }

  function selectAll()   { setThumbs(prev => prev.map(t => ({ ...t, selected: true  }))); }
  function deselectAll() { setThumbs(prev => prev.map(t => ({ ...t, selected: false }))); }

  const selectedCount  = thumbs.filter(t => t.selected).length;
  const renderProgress = totalPages > 0 ? Math.round((thumbsLoaded / totalPages) * 100) : 0;
  ...
  async function handleSplit() {
    ...
    try {
      ...
      if (mode === 'extract') {
        const indices = thumbs.filter(t => t.selected).map(t => t.pageNum - 1);
        const newDoc  = await PDFDocument.create();
        const copied  = await newDoc.copyPages(srcDoc, indices);
        copied.forEach(p => newDoc.addPage(p));
        const bytes = await newDoc.save();
        downloadBlob(new Blob([new Uint8Array(bytes)], { type: 'application/pdf' }), 'extracted-pages.pdf');
        toast.success(`Extracted ${indices.length} page${indices.length !== 1 ? 's' : ''} to PDF`);
        ...
        toast.success(`Split into ${ranges.length} PDF file${ranges.length !== 1 ? 's' : ''}`);
      }
    } catch (err) {
        ...
        {!file && (
          ...
          >
            ...
            <p className="text-slate-400 mb-8 max-w-sm leading-relaxed">
              Extract pages or split a PDF into multiple files — all processed in your browser, nothing uploaded.
            </p>
            ...
            <p className="text-xs text-slate-400 mt-6">PDF files only · Max 100 MB</p>
          </div>
        )}
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
                <div>
                  <span className="text-sm font-bold text-slate-700 truncate max-w-xs inline-block" title={file.name}>
                    {file.name}
                  </span>
                ...
                <div className="flex items-center gap-2">
                  {mode === 'extract' && thumbsLoaded === totalPages && totalPages > 0 && (
                    <>
                      <button onClick={selectAll}   className="text-xs text-slate-500 hover:text-green-600 transition-colors font-medium">All</button>
                      <span className="text-slate-200">|</span>
                      <button onClick={deselectAll} className="text-xs text-slate-500 hover:text-red-500 transition-colors font-medium">None</button>
                      <span className="text-slate-200">|</span>
                    </>
                  )}
                  <button
                    onClick={() => { setFile(null); setThumbs([]); setTotalPages(0); cancelRef.current = true; }}
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-1.5 text-xs text-slate-400">
                    <span className="flex items-center gap-1.5">
                      <Loader2 className="w-3 h-3 animate-spin" /> Rendering thumbnails…
                    </span>
                    <span>{thumbsLoaded}/{totalPages}</span>
                  </div>
              ...
              {thumbs.length > 0 && (
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  {thumbs.map(thumb => (
                    <button
                      key={thumb.pageNum}
                      onClick={() => mode === 'extract' && togglePage(thumb.pageNum)}
            ...
            <div className="lg:w-72 w-full shrink-0">
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 lg:sticky lg:top-6 space-y-5">
                ...
                {mode === 'extract' ? (
                  <div className="space-y-3">
                    <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-100 text-center">
                      <p className="text-2xl font-black text-green-600">{selectedCount}</p>
                      <p className="text-xs text-slate-500 font-medium">
                        page{selectedCount !== 1 ? 's' : ''} selected
                      </p>
                    </div>
                    <p className="text-xs text-slate-400 leading-relaxed text-center">
                      Click thumbnails on the left to select pages. Selected pages will be combined into a single PDF.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      ...
                      <br />
                      <span className="font-medium text-slate-500">Total pages: {totalPages}</span>
                    </p>
                    {rangeInput.trim() && (() => {
                      const ranges = parseRanges(rangeInput, totalPages);
                          ...
                          {ranges.map((pages, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs">
                              ...
                              <span className="text-slate-600">
                                Page{pages.length !== 1 ? 's' : ''} {pages[0]}{pages.length > 1 ? `–${pages[pages.length - 1]}` : ''} ({pages.length}p)
                              </span>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="text-xs text-red-400">No valid ranges found</p>
                      );
                    })()}
                  </div>
                )}
                ...
                <button
                  onClick={handleSplit}
                  disabled={
                    isSplitting || isRendering ||
                    (mode === 'extract' && selectedCount === 0) ||
                    (mode === 'ranges' && parseRanges(rangeInput, totalPages).length === 0)
                  }
                ...
                >
                  {isSplitting ? (
                    ...
                    <><Scissors className="w-4 h-4" /> Extract {selectedCount > 0 ? `${selectedCount} page${selectedCount !== 1 ? 's' : ''}` : 'pages'}</>
                  ) : (() => {
                    const n = parseRanges(rangeInput, totalPages).length;
                    return <><Scissors className="w-4 h-4" /> Split into {n > 0 ? `${n} PDF${n !== 1 ? 's' : ''}` : 'PDFs'}</>;
                  })()}
                </button>
